...
export default function Header() {
  ...
  const ContactButton = ({ mobile = false }) => (
    <a
      ...
      className={
        mobile
          ? "block w-full bg-gradient-to-r from-blue-600 to-emerald-600 text-white text-center py-3 rounded-xl font-semibold hover:from-blue-700 hover:to-emerald-700 transition-all duration-300 shadow-lg"
          : "group relative px-6 py-2.5 bg-gradient-to-r from-blue-600 to-emerald-600 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-emerald-700 transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 flex items-center gap-2 overflow-hidden"
      }
    >
      {!mobile && (
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent transform -skew-x-12 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
      )}
      <span className={mobile ? "flex items-center justify-center gap-2" : ""}>
        Contact{mobile && " Me"}
      </span>
    </a>
  );
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-50 md:hidden transform transition-all duration-300 ${
